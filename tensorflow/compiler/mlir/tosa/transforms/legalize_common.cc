...
std::optional<Value> convertSinOp(PatternRewriter& rewriter, Operation* op,
  ...
  Value fp_scaled =
      CreateMulOpAndInfer(rewriter, op, input_type, input, fp_scale);
  auto floored =
  ...
  auto scale_up = CreateMulOpAndInfer(rewriter, op, input_type, repeated, two);
  auto translate =
  ...
  Value int_limit = getTosaConstTensorSingleF32(
      ...
      input_type.getRank());
  auto int_scaled =
      CreateMulOpAndInfer(rewriter, op, input_type, translate, int_limit);
  ...
  return CreateMulOpAndInfer(rewriter, op, output_type, table_result_fp,
                             output_scale)
      .getResult();
}
